...
do
	HOSPITALS=${HOSPITALS}"hospital__"${i}":"${AB}".HospitalAgent;"
done

ROUTER="router:"${AB}".RoutingAgent;"
export AGENTS="${ROUTER}${HOSPITALS}${PATIENTS}${FIRES}${FIRE_ENGINES}${AMBULANCES}simulatorAgent:org.nkigen.eqr.simulation.SimulationAgent;ECC:"${AB}".EmergencyControlCenterAgent"
